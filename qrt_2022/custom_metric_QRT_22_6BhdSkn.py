...
# use the cosine calue as the metric to measure the similarity
...
def metric(df_y_true, df_y_pred):
    ...
    print(y_pred)
    print(y_true * y_pred)
    print((y_true * y_pred).sum(1))
    print((y_true * y_pred).sum(1).mean())
    mean_overlap = (y_true * y_pred).sum(1).mean()

    return mean_overlap

df_y_true=pd.DataFrame([0,1,1,1,1,1,0])
df_y_pred=pd.DataFrame([0,1,1,1,1,1,0])
print(metric(df_y_true, df_y_pred))